...
const installCloudinaryApp = async (input) => {
  ...
  const [installAppError] = await catchify(
    client.rawRequest({
      method: 'PUT',
      url: `https://api.contentful.com/spaces/${spaceId}/app_installations/zjcnWgBknf9zB7IM9HZjE`,
      data: {
        parameters: {
          cloudName: 'colorful-collective',
          apiKey: '545166997663273',
          maxFiles: 10,
          startFolder: '',
          quality: 'auto',
          format: 'auto',
  ...
  if (installAppError !== null) {
    ...
    return {
      state: 'error',
      error:
        'Failed to install the Cloudinary app - app_installations PUT error error',
    };
  }
  ...
  if (editorInterfacesError !== null) {
    ...
    return {
      state: 'error',
      error:
        'Failed to install the Cloudinary app - editor_interfaces GET error',
    };
  }
  ...
  for (let i = 0; i < contentTypes.length; i += 1) {
      ...
      client.rawRequest({
        ...
        data: {
          ...
          controls: editorInterface.controls.map((control) => {
            if (control.fieldId !== 'externalAsset') {
              return control;
            }
            ...
            return {
              ...
              widgetNamespace: 'app',
              widgetId: 'zjcnWgBknf9zB7IM9HZjE',
            };
          }),
        },
        headers: {
    ...
    if (updateEditorInterfaceError !== null) {
      ...
      return {
        state: 'error',
        error:
          'Failed to install the Cloudinary app - editor_interface PUT error',
      };
    }
  }
...
module.exports = installCloudinaryApp;
